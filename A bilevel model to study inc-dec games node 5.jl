...
GSmin = [22, 27, 32, 37, 42]
GSmax = [24, 29, 34, 39, 44]
...
for t in 1:5
    ...
    function define_sets!(m::Model, data::Dict)
        ...
        new_setupdc = Dict((i, Int[]) for i in N)
        for (bus, generators) in m.ext[:sets][:G_ac]
            new_generatorsupdc = filter(x -> x in GUPDC, generators)
            if !isempty(new_generatorsupdc)
                new_setupdc[bus] = new_generatorsupdc
            end
        end
        G_acupdc = m.ext[:sets][:G_acupdc] = new_setupdc

        new_setupds = Dict((i, Int[]) for i in N)
        for (bus, generators) in m.ext[:sets][:G_ac]
            new_generatorsupds = filter(x -> x in GUPDS, generators)
            if !isempty(new_generatorsupds)
                new_setupds[bus] = new_generatorsupds
            end
        end
        G_acupds = m.ext[:sets][:G_acupds] = new_setupds
        ...
        new_setdotc = Dict((i, Int[]) for i in N)
        for (bus, generators) in m.ext[:sets][:G_ac]
            new_generatorsdotc = filter(x -> x in GDOTC, generators)
            if !isempty(new_generatorsdotc)
                new_setdotc[bus] = new_generatorsdotc
            end
        end
        G_acdotc = m.ext[:sets][:G_acdotc] = new_setdotc

        new_setdots = Dict((i, Int[]) for i in N)
        for (bus, generators) in m.ext[:sets][:G_ac]
            new_generatorsdots = filter(x -> x in GDOTS, generators)
            if !isempty(new_generatorsdots)
                new_setdots[bus] = new_generatorsdots
            end
        end
        G_acdots = m.ext[:sets][:G_acdots] = new_setdots
    ...
    function build_model!(m::Model)
        ...
        G_acupdc = m.ext[:sets][:G_acupdc] 
        G_acupds = m.ext[:sets][:G_acupds] 
        G_acdod = m.ext[:sets][:G_acdod]
        G_acupt = m.ext[:sets][:G_acupt]
        G_acdot = m.ext[:sets][:G_acdot]
        G_acdots = m.ext[:sets][:G_acdots]
        G_acdotc = m.ext[:sets][:G_acdotc]
        ...
        pfup = m.ext[:variables][:pfup] = @variable(m, [g=GUPC], lower_bound = 0 , base_name = "pfupc")
        ...
        prdo = m.ext[:variables][:prdo] = @variable(m, [g=GDOTC], lower_bound = 0 , base_name = "prdo")
        ...
        xrdo = m.ext[:variables][:xrdo] = @variable(m, [gs=GDOTS,k=K], Bin, base_name = "xrdo")
        prdos = m.ext[:expressions][:prdos] = @expression(m, [g=GDOTS], gamma*sum((2^k)*xrdo[g,k] for k in K))
        ...
        m.ext[:objectives][:Obj] = @objective(m, Min, sum(cg[gs]*pw[gs] for gs in GS) + sum(cg[gc]*pw[gc] for gc in GC) - lambdaw*sum(pd[l] for l in L) + sum(pmax[gc]*mu_wsmup[gc] for gc in GC) - sum(lambdaf[5]*gamma*sum((2^k)*xfup[gs,k] for k in K) for gs in GUPS) + sum(cg[gs]*gamma*sum((2^k)*xfup[gs,k] for k in K) for gs in GUPS)+ sum(lambdar[4]*gamma*sum((2^k)*xrdo[gs,k] for k in K) for gs in GDOTS) - sum(cg[gs]*gamma*sum((2^k)*xrdo[gs,k] for k in K) for gs in GDOTS))
        ...
        m.ext[:constraints][:p_balanced5] = @constraint(m, -(sum(pw[g] + pfup[g]  for g in GUPT) - pd[1]) -sum(pw[g] for g in G_ac[5]) - sum(gamma*sum((2^k)*xfup[g,k] for k in K) for g in G_acupds[5]) - sum(pfup[g] for g in G_acupdc[5]) + sum(pfdo[g] for g in G_acdod[5]) + sum(pd[l] for l in L_ac[5]) + sum(pb[(b,i,j)] for (b,i,j) in B_arcs_frd[5]) - sum(pb[(b,i,j)] for (b,i,j) in B_arcs_tod[5]) == 0)
        ...
        m.ext[:constraints][:Fr1as] = @constraint(m, [g=GUPTS], prup[g] <= M*(1-br1[g]))
        m.ext[:constraints][:Fr1bs] = @constraint(m, [g=GUPTS], choedrup[g] + mu_rup[g] - sum(lambdar[i] for i in N_act[g]) <= M*br1[g])
        m.ext[:constraints][:Fr2up] = @constraint(m, [g=GUPT], 0 <= pmax[g]- pw[g] - pfup[g] - prup[g])
        m.ext[:constraints][:Fr2a] = @constraint(m, [g=GUPT], mu_rup[g]  <= M*(1-br2[g]))
        m.ext[:constraints][:Fr2bup] = @constraint(m, [g=GUPT], pmax[g]- pw[g] - pfup[g] - prup[g] <= M*br2[g])
        ...
        m.ext[:constraints][:Fr4do] = @constraint(m, [g=GDOTC], 0 <= pw[g] + pfup[g] - prdo[g])
        m.ext[:constraints][:Fr4a] = @constraint(m, [g=GDOTC], mu_rdo[g]  <= M*(1-br4[g]))
        m.ext[:constraints][:Fr4bdo] = @constraint(m, [g=GDOTC], pw[g] + pfup[g] - prdo[g] <= M*br4[g])

        m.ext[:constraints][:Fr3s] = @constraint(m, [g=GDOTS], 0 <= -choedrdo[g] + mu_rdo[g] + sum(lambdar[4]))
        m.ext[:constraints][:Fr3as] = @constraint(m, [g=GDOTS], gamma*sum((2^k)*xrdo[g,k] for k in K) <= M*(1-br3[g]))
        m.ext[:constraints][:Fr3bs] = @constraint(m, [g=GDOTS], -choedrdo[g] + mu_rdo[g] + sum(lambdar[4]) <= M*br3[g])
        m.ext[:constraints][:Fr4dos] = @constraint(m, [g=GDOTS], 0 <= pw[g] + gamma*sum((2^k)*xfup[g,k] for k in K) - gamma*sum((2^k)*xrdo[g,k] for k in K))
        m.ext[:constraints][:Fr4as] = @constraint(m, [g=GDOTS], mu_rdo[g]  <= M*(1-br4[g]))
        m.ext[:constraints][:Fr4bdos] = @constraint(m, [g=GDOTS], pw[g] + gamma*sum((2^k)*xfup[g,k] for k in K) - gamma*sum((2^k)*xrdo[g,k] for k in K) <= M*br4[g])
        ...
        m.ext[:constraints][:p_balancet4] = @constraint(m, -pb[(4,3,4)] + pb[(5,5,6)] + pb[(7,5,8)] - sum(pw[g] for g in G_ac[5]) - sum(gamma*sum((2^k)*xfup[g,k] for k in K) for g in G_acupds[5]) - sum(pfup[g] for g in G_acupdc[5]) + sum(gamma*sum((2^k)*xrdo[g,k] for k in K) for g in G_acdots[5]) + sum(prdo[g] for g in G_acdotc[5]) == 0)
        m.ext[:constraints][:p_balancet] = @constraint(m, [i in 1:3], -sum(pw[g] for g in G_ac[i]) - sum(pfup[g] for g in G_acupt[i]) - sum(prup[g] for g in G_acupt[i]) + sum(pfdo[g] for g in G_acdot[i]) + sum(prdo[g] for g in G_acdot[i]) + sum(pd[l] for l in L_ac[i]) + sum(pb[(b,i,j)] for (b,i,j) in B_arcs_frt[i]) - sum(pb[(b,i,j)] for (b,i,j) in B_arcs_tot[i]) == 0)
