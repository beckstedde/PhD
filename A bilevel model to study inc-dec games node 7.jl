...
for t in 3:3
    ...
    function define_sets!(m::Model, data::Dict)
        ...
        new_setdodc = Dict((i, Int[]) for i in N)
        for (bus, generators) in m.ext[:sets][:G_ac]
            new_generatorsdodc = filter(x -> x in GDODC, generators)
            if !isempty(new_generatorsdodc)
                new_setdodc[bus] = new_generatorsdodc
            end
        end
        G_acdodc = m.ext[:sets][:G_acdodc] = new_setdodc

        new_setdods = Dict((i, Int[]) for i in N)
        for (bus, generators) in m.ext[:sets][:G_ac]
            new_generatorsdods = filter(x -> x in GDODS, generators)
            if !isempty(new_generatorsdods)
                new_setdods[bus] = new_generatorsdods
            end
        end
        G_acdods = m.ext[:sets][:G_acdods] = new_setdods
    ...
    function build_model!(m::Model)
        ...
        G_acdods = m.ext[:sets][:G_acdods]
        G_acdodc = m.ext[:sets][:G_acdodc]
        ...
        pfup = m.ext[:variables][:pfup] = @variable(m, [g=GUP], lower_bound = 0 , base_name = "pfup")
        pfdo = m.ext[:variables][:pfdo] = @variable(m, [g=GDODC], lower_bound = 0 , base_name = "pfdo")
        ...
        xfdo = m.ext[:variables][:xfdo] = @variable(m, xfdo[gs=GDODS,k=K], Bin, base_name = "xfdo")
        pfdos = m.ext[:expressions][:pfdos] = @expression(m, [g=GDODS], gamma*sum((2^k)*xfdo[g,k] for k in K))
        ...
        m.ext[:objectives][:Obj] = @objective(m, Min, sum(cg[gs]*pw[gs] for gs in GS) + sum(cg[gc]*pw[gc] for gc in GC) - lambdaw*sum(pd[l] for l in L) + sum(pmax[gc]*mu_wsmup[gc] for gc in GC) + sum(lambdaf[7]*gamma*sum((2^k)*xfdo[gs,k] for k in K) for gs in GDODS) - sum(cg[gs]*gamma*sum((2^k)*xfdo[gs,k] for k in K) for gs in GDODS))
        ...
        m.ext[:constraints][:F1atds] = @constraint(m, [g=GUPTS], pfup[g] <= M*(1-b1[g]))
        ...
        m.ext[:constraints][:F1as] = @constraint(m, [g=GUPDS], pfup[g] <= M*(1-b1[g]))
        m.ext[:constraints][:F1bs] = @constraint(m, [g=GUPDS], choedfup[g] + mu_fup[g] - sum(lambdaf[i] for i in N_acd[g]) <= M*b1[g])
        m.ext[:constraints][:F2] = @constraint(m, [g=GUP], 0 <= pmax[g]- pw[g] - pfup[g])
        m.ext[:constraints][:F2a] = @constraint(m, [g=GUP], mu_fup[g]  <= M*(1-b2[g]))
        m.ext[:constraints][:F2b] = @constraint(m, [g=GUP], pmax[g]- pw[g] - pfup[g] <= M*b2[g])
        ...
        m.ext[:constraints][:F4] = @constraint(m, [g=GDODC], 0 <= pw[g] - pfdo[g])
        m.ext[:constraints][:F4a] = @constraint(m, [g=GDODC], mu_fdo[g]  <= M*(1-b4[g]))
        m.ext[:constraints][:F4b] = @constraint(m, [g=GDODC], pw[g] - pfdo[g] <= M*b4[g])

        m.ext[:constraints][:F3s] = @constraint(m, [g=GDODS], 0 <= -choedfdo[g] + mu_fdo[g] + sum(lambdaf[i] for i in N_acd[g]))
        m.ext[:constraints][:F3as] = @constraint(m, [g=GDODS], gamma*sum((2^k)*xfdo[g,k] for k in K) <= M*(1-b3[g]))
        m.ext[:constraints][:F3bs] = @constraint(m, [g=GDODS], -choedfdo[g] + mu_fdo[g] + sum(lambdaf[i] for i in N_acd[g]) <= M*b3[g])
        m.ext[:constraints][:F4s] = @constraint(m, [g=GDODS], 0 <= pw[g] - gamma*sum((2^k)*xfdo[g,k] for k in K))
        m.ext[:constraints][:F4as] = @constraint(m, [g=GDODS], mu_fdo[g]  <= M*(1-b4[g]))
        m.ext[:constraints][:F4bs] = @constraint(m, [g=GDODS], pw[g] - gamma*sum((2^k)*xfdo[g,k] for k in K) <= M*b4[g])
        ...
        m.ext[:constraints][:p_balanced5] = @constraint(m, -(sum(pw[g] + pfup[g]  for g in GUPT) - pd[1]) -sum(pw[g] for g in G_ac[5]) - sum(pfup[g] for g in G_acupd[5]) + sum(pfdo[g] for g in G_acdod[5]) + sum(pd[l] for l in L_ac[5]) + sum(pb[(b,i,j)] for (b,i,j) in B_arcs_frd[5]) - sum(pb[(b,i,j)] for (b,i,j) in B_arcs_tod[5]) == 0)
        m.ext[:constraints][:p_balance] = @constraint(m, [i in 6:9], -sum(pw[g] for g in G_ac[i]) - sum(pfup[g] for g in G_acupd[i]) + sum(pfdo[g] for g in G_acdodc[i]) + sum(gamma*sum((2^k)*xfdo[g,k] for k in K) for g in G_acdods[i]) + sum(pd[l] for l in L_ac[i]) + sum(pb[(b,i,j)] for (b,i,j) in B_arcs_frd[i]) - sum(pb[(b,i,j)] for (b,i,j) in B_arcs_tod[i]) == 0)
